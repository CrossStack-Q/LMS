...
export default async function Blog() {
  ...
  return (
    <div className="max-w-7xl mx-auto border-x border-b border-(--gray-500)">
      ...
      {/* HEADER */}
      <div className="
        px-4 sm:px-6 pt-8 sm:pt-10 pb-4
        flex flex-col sm:flex-row
        gap-4
        sm:items-end sm:justify-between
      ">
        <div>
          <p className="text-2xl sm:text-3xl font-semibold">Blogs</p>

          <div className="flex gap-4 mt-2 text-sm text-(--gray-700)">
            <button className="hover:text-(--gray-900)">Newest</button>
            <button className="hover:text-(--gray-900)">Popular</button>
          </div>
        </div>
        ...
        {blogs.map((blog, index) => (
          ...
          >
            <div className="p-4 sm:p-6 flex flex-col gap-2">
              ...
              {/* IMAGE */}
              <div className="h-40 sm:h-48 flex justify-center overflow-hidden rounded">
                <img
                  src={blog.image}
                  alt={blog.title}
                  className="h-full"
